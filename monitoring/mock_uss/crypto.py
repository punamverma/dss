...
sig_params = '"{}": ({});created=1666654644774'.format(
    "@signature-params",
    " ".join(list(map(lambda comp: '"{}"'.format(comp), covered_components))),
)
rsb = rsb + sig_params
print(
    "HASH OF RSB: "
    + base64.b64encode(hashlib.sha512(rsb.encode("utf-8")).digest()).decode("utf-8")
)
print(repr(rsb))
